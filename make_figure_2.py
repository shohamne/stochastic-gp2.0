...
def _choose_grad_column(df, override: str | None) -> str:
    """
    Decide which column in the DataFrame contains ||∇ℓ(θ)||².

    The column must be explicitly specified via --grad-column.
    If omitted, we expect the column to be named "||grad||".
    """
    column = override or "grad_norm"
    if column not in df.columns:
        raise ValueError(
            f"Requested grad-column '{column}' not found. "
            f"Available columns: {list(df.columns)}"
        )
    return column


def make_figure2_from_df(
    df,
    output: str | Path = "neurips_figure2_all_algos.png",
    grad_column: str | None = None,
    smooth_window: int = 1,
) -> None:
    """
    ...
    Each panel shows mean and ±1 standard error of
        log10 ||∇ℓ(θ^(k))||_2^2
    versus iteration, across seeds, starting from θ^(0)=(5.0,3.0).

    We use the short, 25-epoch runs whose iteration counts
    match NeurIPS Figure 2.
    """
    ...
    grad_column = _choose_grad_column(df, grad_column)
    ...
    for row_idx, algo in enumerate(ALGO_ORDER):
        for col_idx, m in enumerate(BATCH_SIZES):
            ...
            # Pivot into a (iter × seed) table of gradient norms.
            # If multiple rows per (iter, seed) appear, we take the last.
            ...
            # Guard against non-positive values before taking log10.
            values[values <= 0] = np.nan

            # Mean and SE in log10 space
            log_vals = np.log10(values)
            mean_log = np.nanmean(log_vals, axis=1)
            n_eff = np.sum(np.isfinite(log_vals), axis=1)
            with np.errstate(invalid="ignore", divide="ignore"):
                se_log = np.nanstd(log_vals, axis=1, ddof=1) / np.sqrt(n_eff)

            iters = pivot.index.to_numpy(dtype=float)

            mean_log = smooth_1d(mean_log)
            se_log = smooth_1d(se_log)

            lower = mean_log - se_log
            upper = mean_log + se_log

            ax.plot(iters, mean_log, color="C0", linewidth=1.5)
            ax.fill_between(
                iters,
                lower,
                upper,
            ...
            # Column titles: minibatch size
            if row_idx == 0:
                ax.set_title(f"$m = {m}$")

            # Row labels: algorithm name + y-axis title
            if col_idx == 0:
                ax.set_ylabel(
                    ALGO_TITLES[algo]
                    + "\n"
                    + r"$\log_{10}\|\nabla \ell(\theta^{(k)})\|_2^2$",
                    fontsize=10,
                )
    ...
    # Shared y-limits across all visible axes
    ...
    print(f"[Figure 2] Saved plot to {output}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Reconstruct NeurIPS Figure 2 (full-gradient convergence vs minibatch size) "
            "for BSGD, MINIMAX and SCGD from stdout logs."
        )
    )
    parser.add_argument(
        "--root",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("neurips_figure2_all_algos.png"),
    ...
    parser.add_argument(
        ...
        default="||grad||",
        help=(
            "Name of the column containing ||∇ℓ(θ)||^2 "
            "(default: '||grad||')."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "mean and standard-error curves before plotting (default: 1, no smoothing)."
        ),
    )
    return parser
