...
def make_figure2_real_nlml_from_df(
    df,
    output: str | Path = "neurips_figure2_real_nlml.png",
    nlml_column: str | None = None,
    smooth_window: int = 1,
) -> None:
    """
    Create a 3x3 panel plot mirroring NeurIPS Figure 2 but for the real NLML:

        rows    = algorithms (BSGD, MINIMAX, SCGD)
        columns = minibatch sizes m = 64, 32, 16

    Each panel shows mean and ±1 standard error of the real NLML
    ...
    # Figure 2 uses θ^(0) = (5.0, 3.0) and (for BSGD) α1 = 9.0 and 25-epoch
    # runs with batch sizes m in {16, 32, 64}. We will further split by batch
    # size per column.
    df = df[
        (df["sigma_f2_init"] == 5.0)
    ...
    for row_idx, algo in enumerate(ALGO_ORDER):
        for col_idx, m in enumerate(BATCH_SIZES):
            ...
            # Pivot into a (iter × seed) table of NLML values.
    ...
    print(f"[Figure 2 - Real NLML] Saved plot to {output}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "but for the real NLML trajectories."
        )
    )
    parser.add_argument(
        "--root",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("neurips_figure2_real_nlml.png"),
...
def main() -> None:
    ...
    make_figure2_real_nlml_from_df(
        df,
        output=args.output,
